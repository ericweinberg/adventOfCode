...
public class Day8 {
    ...
    private static Logger logger = LoggerFactory.getLogger(Day8.class);
    ...
    public static void main(String[] args) {
        ...
        Day8Result result = day8.stripEscapedCharacters(strings);
        logger.info("After stripping escaped characters, the number of codeCharacters is {} and the number of characters in memory is {}, the difference is {}",
                result.getNumberOfCodeCharacters(), result.getNumberOfMemoryCharacters(),
                result.getNumberOfCodeCharacters()-result.getNumberOfMemoryCharacters());

        //part 2 solution
        result = day8.escapeCharacters(strings);
        logger.info("After escaping characters, the number of codeCharacters is {} and the number of characters in memory is {}, the difference is {}",
                result.getNumberOfCodeCharacters(), result.getNumberOfMemoryCharacters(),
                result.getNumberOfMemoryCharacters()-result.getNumberOfCodeCharacters());
    }


    public Day8Result escapeCharacters(List<String> strings) {

        int numberOfMemoryCharacters = 0;
        int numberOfCodeCharacters = 0;
        ...
        for(String string : strings) {
            ...
            string = string.replaceAll("\"", "\\\\\"");
            string="\""+string+"\"";
            numberOfMemoryCharacters+=string.length();
        }

        return new Day8Result(numberOfCodeCharacters,numberOfMemoryCharacters);
    }

    public Day8Result stripEscapedCharacters(List<String> strings) {
        ...
        for(String string : strings) {
            numberOfCodeCharacters+=string.length();
            string = string.replaceAll("\\\\\\\\", "/");
            string = string.replaceAll("\\\\x\\w{2}", "'");
            string = string.replaceAll("\\\\\"", "'");
            string = string.replaceAll("\"", "");
            numberOfMemoryCharacters+=string.length();
        }
